...
* | in ``MESSAGE_framework/scaling_investment_costs.gms``.
*
***
...
*        & + fix\_cost_{n,t,y,y} \cdot \frac{ 1 }{ \sum_{h'} duration^H_{h'} \cdot capacity\_factor_{n,t,y,y,h'} } \\
*        & + var\_cost_{n,t,y,y,m,h}
*
* where :math:`|y| = technical\_lifetime_{n,t,y}`. This formulation is a direct re-implementation of `MESSAGE V`;
* note that it assumes constant fixed and variable costs over time.
*
* **Important comment:** All soft relaxations of the dynamic activity constraint are
...
* compute discounted annualized investment costs if interest rate > 0
            ( interestrate(year)
                * POWER( 1 + interestrate(year) , technical_lifetime(node,tec,year) )
                / ( POWER( 1 + interestrate(year) , technical_lifetime(node,tec,year) ) - 1 )
              )$( interestrate(year) )
...
construction_time_factor(node,inv_tec,year)$( map_tec(node,inv_tec,year) AND construction_time(node,inv_tec,year) ) =
    ( POWER( 1 + interestrate(year), construction_time(node,inv_tec,year) ) ) ;
...
*        economic\_lifetime_{n,t,y^V} - \sum_{y' \geq y^V} duration^Y_{y'} \Big\}
*
* .. math::
...
* :math:`|\widehat{y}| = period\_duration^Y_{\widehat{y}}`
